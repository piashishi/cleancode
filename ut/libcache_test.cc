    ...
    do
    {
        ...
        if (entry == NULL)
        {
            if(key.imsi.val.imsi64bit == 0)
                break;
            printf("libcache_lookup after insertion, ERROR! key = %llu\n",
                    key.imsi.val.imsi64bit);
            return TEST_FAILURE;
        }
        libcache_unlock_entry(libcache, entry);
        ...
        if (entry != NULL)
        {
            printf("hashtable_delete_by_node ERROR! key = %llu\n",
                    key.imsi.val.imsi64bit);
            return TEST_FAILURE;
        }
        key.imsi.val.imsi64bit--;
